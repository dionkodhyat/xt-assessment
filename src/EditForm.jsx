...
function validateValue(value, attributeName, validationType) {
  ...
  switch (validationType) {
    ...
    case ValidationType.Number:
      ...
      break;
    case ValidationType.PillList:
      if (!value) {
        toast.error(attributeName + " is invalid. Please enter at least one value");
        return false;
      }
      break;
    default: break;
...
export function EditForm(props) {
  ...
  const [pillLists, setPillLists] = useState(
      props.editEntries.reduce((acc, editEntry) => {
        if (editEntry.type === EditEntryType.PillList) {
          acc[editEntry.attribute] = props.entityObj[editEntry.attribute] || [];
        }
        return acc;
      }, {})
  );
  const [pillInput, setPillInput] = useState("");
  const addPill = (attribute, value) => {
    if (!value.trim()) return;
    const updatedPillList = [...pillLists[attribute], value.trim()];
    setPillLists((prev) => ({ ...prev, [attribute]: updatedPillList }));
    entity[attribute] = updatedPillList;
  };

  const removePill = (attribute, indexToRemove) => {
    const updatedPillList = pillLists[attribute].filter((_, index) => index !== indexToRemove);

    setPillLists((prev) => ({ ...prev, [attribute]: updatedPillList }));

    if (updatedPillList.length === 0) {
      const updatedEntity = { ...entity };
      delete updatedEntity[attribute];
      setEntity(updatedEntity);
    } else {
      setEntity((prev) => ({ ...prev, [attribute]: updatedPillList }));
    }
  };
                    ...
                    return (
                        <div className="col-span-6">
                          <label className="block text-sm font-medium text-gray-700">
                            <h3 className="text-lg font-medium leading-6 text-gray-900 mb-2">{editEntry.attributeName + requiredMark}</h3>
                            {editEntry.subName && (
                                <p className="text-sm text-gray-500">{editEntry.subName}</p>
                            )}
                          </label>
                          <div className="my-4 border-1 flex flex-wrap gap-2" style={{minHeight: "30px"}}>
                            {pillLists[editEntry.attribute].map((pill, index) => (
                                <div className="border rounded-sm flex items-center">
                                  <span
                                      key={index}
                                      className="text-sky-600 pl-1 py-1 text-sm font-medium"
                                  >
                                  {pill}
                                  </span>
                                  <span
                                      className="mx-1 cursor-pointer text-gray-500 font-weight-500 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-300 rounded"
                                      onClick={() => removePill(editEntry.attribute, index)}
                                  >
                                    <svg className="h-4 w-4 text-gray-400 hover:text-gray-600" fill="none"
                                         viewBox="0 0 24 22" stroke="currentColor">
                          ...
                          <div className="flex mt-2 items-center gap-2 text-sm text-gray-500">
                            <div className="flex">
                              <input
                                  type="text"
                                  ...
                                  onKeyDown={(e) => {
                                    if (e.key === 'Enter') {
                                      e.preventDefault();
                                      if (pillInput.trim()) {
                                        addPill(editEntry.attribute, pillInput.trim());
                                        setPillInput("");
                                      }
                                    }
                                  }}
                            ...
                            <div>
                              <button
                                  ...
                                  type="button"
                                  aria-label="Add pill"
                                  className={`rounded-md border py-1 px-4 text-sm font-medium shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                                      pillInput
